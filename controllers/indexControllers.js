...
const {
  sendmail_Partner_us,
  sendmail_For_WeeklyPerformace_On_Sunday,
} = require("../utils/sendEmail");

const stripe = require("stripe")(
  "sk_test_51PSzUZRtcNodwMW8VfMiPY8LlcQRHuvrsoA6EcARF2ewjWguoOCJvqNcjkQ8ZBEZohJ91LadCYbOqcv128chGMhe00QwpqVyhU"
);
// ---------------------------------------------------------------------------------------------------------- Image Upload Route ----------------------
...
exports.Signup = async (req, res, next) => {
  try {
    const { Username, Email, Password, Children_Name, FromGoogle } = req.body;
...
exports.Edit_Profile = catchAsyncErrors(async (req, res, next) => {
  try {
    ...
    return res
      .status(200)
      .json({ message: "Profile updated successfully", Existing_Student });
  } catch (error) {
    console.log(error);
    return res.status(500).json({ message: "Internal Server Error" });
  }
  ...
  try {
    ...
    if (passwordMatch) {
      ...
      return res.status(500).json({
        message: "Please give a Valid Current Password to Change the Password",
      });
    }
  } catch (error) {
    console.log(error);
...
exports.Fetch_Partner_With_Us_Data = catchAsyncErrors(
  async (req, res, next) => {
    try {
      console.log(req.body);
      const { Email, subject, message } = req.body;
      try {
        await sendmail_Partner_us(Email, subject, message);
...
async function SendCurrentWeekProgress_IF_TODAY_IS_Sunday() {
  try {
    const today = new Date();
    const todayDayOfWeek = today.getDay();

    let previousWeekStartDate, previousWeekEndDate;

    if (todayDayOfWeek === 0) {
      // If today is Sunday
      previousWeekEndDate = new Date(today);

      previousWeekStartDate = new Date(previousWeekEndDate);
      previousWeekStartDate.setDate(previousWeekEndDate.getDate() - 7);
      console.log(previousWeekStartDate, "-------previousWeekStartDate");
    }

    const studentWeeklyPerformances = await Weeklyperformance.find().populate(
      "StudentId"
    );
    ...
    for (const WeeklyPerformance of studentWeeklyPerformances) {
      ...
      if (
        ...
        previousWeekEndDate &&
        WeeklyPerformancesStartingDate <= previousWeekEndDate &&
        WeeklyPerformancesEndingDate >= previousWeekStartDate
      ) {
        await sendmail_For_WeeklyPerformace_On_Sunday(
          WeeklyPerformance.StudentId.Email,
          WeeklyPerformance
        );

        // Weekly performance falls within the previous week
        // Make Condition For All the Dates between the Start and End of the week.
        // const AllDates_Between_StartandEnd = [];
        // let currentDate = new Date(WeeklyPerformancesStartingDate);
        // while (currentDate <= WeeklyPerformancesEndingDate) {
        ...
        // (Add your additional logic here to handle the dates)
      }
    }
  } catch (error) {
    console.error("Error updating progress records:", error);
...
//       `Running the update progress function at UK midnight: ${currentUKTime}`
//     );
//     await SendCurrentWeekProgress_IF_TODAY_IS_Sunday();
//   },
//   {
//     timezone: "Europe/London",
//   }
...
exports.Fetch_FreePayment = catchAsyncErrors(async (req, res) => {
  try {
    console.log(req.body);
    const { products } = req.body;
    console.log(products);

    const unitAmountInPence = Math.round(products.Payprice * 100);
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      line_items: [
        {
          price_data: {
            currency: "gbp", // Change currency to GBP (British Pounds)
            product_data: {
              name: products.title, // replace with your product name
            },
            unit_amount: unitAmountInPence, // Stripe requires the amount to be in cents
          },
          quantity: 1,
        },
      ],
      mode: "payment",
      metadata: {
        student_id: products.id, // Adding the student ID to metadata
        Plan_Name: products.title,
        isFree: products.isFree,
      },
      success_url:
        "https://667566c66fa1e000087f0240--idyllic-hummingbird-f60be5.netlify.app/successPayment",
      cancel_url:
        "https://667566c66fa1e000087f0240--idyllic-hummingbird-f60be5.netlify.app/cancelPayment",
    });
...
exports.Fetch_PaymentDetails = catchAsyncErrors(async (req, res) => {
  try {
    console.log(req.body);
    const { products } = req.body;
    console.log(products);

    // Assuming products.priceId is actually a valid price ID
    const priceId = products.priceId;

    if (!priceId) {
      return res
        .status(500)
        .json({ message: "Price ID is missing from the request" });
    }
...
exports.Activate_BuyPlan = catchAsyncErrors(async (req, res) => {
  try {
    console.log(req.body);
    const { student_id, created, sessionID, Plan_Name, isFree } = req.body;
    const Existing_user = await Student.findById(student_id);
    ...
    console.log(isFree);
    Existing_user.buyPlan = true;
    Existing_user.isFreePlan = true;
    Existing_user.Active_Plan = Plan_Name;
    ...
    Existing_user.PaymentsID = PaymentDetails;

    await Existing_user.save();

    res
      .status(200)
      .json({ message: "Buy plan activated successfully", Existing_user });
  } catch (error) {
    console.error("Error in Activate_BuyPlan:", error);
    return res.status(500).json({ message: "Internal Server Error" });
  }
});
...
exports.GetSessionDetails = catchAsyncErrors(async (req, res) => {
  const { id } = req.params;
  try {
...
async function Make_Plan_inActive() {
  try {
    // Await the result of the find query
    const Find_All_Active_User = await Student.find({ buyPlan: true });
    console.log(Find_All_Active_User);

    for (const Active_User of Find_All_Active_User) {
      // Ensure PaymentsID is an array and has elements
      if (Active_User.PaymentsID && Active_User.PaymentsID.length > 0) {
        // Access the createdDate of the first payment
        const FreeSession_startDate = new Date(
          Active_User.PaymentsID[0].createdDate
        );
        ...
        console.log(Active_User.Active_Plan);
        // Check if the difference is greater than 365 days  - Yearly
        if (diffInDays > 365 && Active_User.Active_Plan === "Yearly") {
          Active_User.buyPlan = false;
          Active_User.Active_Plan = "";

          // Save the updated user
          await Active_User.save();
          console.log(
            `Updated user ${Active_User._id}: buyPlan set to false, Active_Plan cleared from Yearly `
          );
        }

        // Check if the difference is greater than 182 days - Half-Yearly
        if (diffInDays > 182 && Active_User.Active_Plan === "Half-Yearly") {
          Active_User.buyPlan = false;
          Active_User.Active_Plan = "";
          ...
          await Active_User.save();
          console.log(
            `Updated user ${Active_User._id}: buyPlan set to false, Active_Plan cleared from Half-Yearly `
          );
        }

        // Check if the difference is greater than 30 days - Monthly
        if (diffInDays > 30 && Active_User.Active_Plan === "Monthly") {
          Active_User.buyPlan = false;
          Active_User.Active_Plan = "";
          ...
          await Active_User.save();
          console.log(
            `Updated user ${Active_User._id}: buyPlan set to false, Active_Plan cleared from Monthly `
          );
        }

        // Check if the difference is greater than 10 days
        if (diffInDays > 10 && Active_User.Active_Plan === "Free-Trial") {
          Active_User.buyPlan = false;
          Active_User.Active_Plan = "";
          ...
          await Active_User.save();
          console.log(
            `Updated user ${Active_User._id}: buyPlan set to false, Active_Plan cleared From Free Trial`
          );
        }
      } else {
        console.log(
          "No PaymentsID or empty PaymentsID array for user:",
          Active_User
        );
      }
    }
...
exports.Cancel_Current_Plan = catchAsyncErrors(async (req, res, next) => {
  try {
    console.log(req.body)
    ...
    // Check if the session contains a subscription ID
    ...
    Existing_Student.buyPlan = false;
    Existing_Student.Active_Plan = "";
    await Existing_Student.save();

    return res.status(200).json({
      message: "Your Current Plan is Deactivated Successfully",
      Existing_Student,
    });
  } catch (error) {
    console.log(error);
